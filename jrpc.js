...
const banner = cfonts.render(("HG REST APIS"), {
font: "block",
align: "center",
...
await listkeys(apikey, req);
url = req.query.url
 if (!url) return res.json({ status : false, criador : `@HG`, mensagem : "Coloque o parametro: url"})
fbdown(url).then(data => {
res.json({
status: true,
mensagem: 'HG REST APPIS - MELHOR SITE DE APIS',
criador: `@HG - 37 99920752`,
resultado: data
})}).catch(e => {
res.json({
...
if(!key.map(i => i.apikey)?.includes(apikey))return res.sendFile(path.join(__dirname, "./public/", "limited.html"))
await listkeys(apikey, req);
if (!q) return res.json({ status : false, creator : `@HG`, message : "Cade o parametro q?"}) 
...
await listkeys(apikey, req);
q = req.query.q
 if (!q) return res.json({ status : false, criador : `@HG`, mensagem : "Coloque o parametro: q"})
...
await listkeys(apikey, req);
nome = req.query.nome
 if (!nome) return res.json({ status : false, criador : `@HG`, mensagem : "Coloque o parametro: nome"})
...
await listkeys(apikey, req);
signo = req.query.signo
if (!signo) return res.json({ status : false, criador : `@HG`, mensagem : "Coloque o parametro: signo"})
...
categoria = req.query.categoria
 if (!categoria) return res.json({ status : false, criador : `@HG`, mensagem : "Coloque o parametro: categoria"})
...
pergunta = req.query.pergunta
 if (!pergunta) return res.json({ status : false, criador : `@HG`, mensagem : "Coloque o parametro: pergunta"})
...
texto = req.query.texto
 if (!texto) return res.json({ status : false, criador : `@HG`, mensagem : "Coloque o parametro: texto"})
...
if(!key.map(i => i.apikey)?.includes(apikey))return res.sendFile(path.join(__dirname, "./public/", "limited.html"))
await listkeys(apikey, req);
  if (!query) return res.json({ status : false, criador : `@HG`, mensagem : "Coloque o parametro: query"})
...
if (!url) return res.json({ status : false, creator : `@HG`, message : "Cade o parametro url?"}) 
        if(key.includes(apikey)){
       igstory(url)
	.then(data => {
		var result = data;
...
await listkeys(apikey, req);
text = req.query.text
if (!text) return res.json({ status : false, creator : `@HG`, message : "Cade o parametro text?"})
        if(key.includes(apikey)){
...
await listkeys(apikey, req);
query = req.query.query
if (!query) return res.json({ status : false, creator : `@HG`, message : "Cade o parametro query?"})
wikimedia(query)
.then(hasil => {
var resultado = hasil;
res.json({
...
await listkeys(apikey, req);
url = req.query.url
if (!url) return res.json({ status : false, criador : `@HG`, mensagem : "Coloque o parametro: q"})
...
status = {
    ...
    request: totalreq,
        ...
        info:{       
            owner: 'VNCSCODE',
            whatsapp: 'https://wa.me/553799920752'
        }
    }
res.json(status)
}});
